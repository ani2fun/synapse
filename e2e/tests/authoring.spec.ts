...
test("every d2 figure carries the fence it came from", async ({ request }) => {
  ...
  const at = [...body.matchAll(/data-fence-at="(\d+)"/g)].map((m) => Number(m[1]));
...
test("mermaid figures get no Edit pill — it is a different language", async ({ page }) => {
  await page.goto(LESSON);
  await expect(page.locator(".mermaid-block .diagram").first()).toBeVisible({ timeout: 15_000 });
  await expect(page.locator(".mermaid-block .diagram__edit")).toHaveCount(0);
  // …but it still enlarges, so this is an absence by design rather than a broken card.
